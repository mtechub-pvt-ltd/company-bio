...
function Requests({id}) {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        ...
        const requestsObj = result.data?.data?.requests?.recent_activity || [];
        const transformed = requestsObj.map((item, idx) => ({
          id: idx + 1,
          type: item.type,
          status: item.status,
  ...
  // Fetch all requests for export (no pagination)
  const fetchAllRequestsData = async () => {
    try {
      ...
      const result = await res.json();
      const requestsObj = result.data?.data?.requests?.recent_activity || [];
      return requestsObj.map((item, idx) => ({
        ID: idx + 1,
        Type: item.type,
        Status: item.status,
  ...
  const handleExportData = async (format) => {
    ...
    try {
      const allData = await fetchAllRequestsData();
      if (!allData.length) {
      ...
      if (format.toLowerCase() === "pdf") {
        ...
        await exportTable(allData, "Requests", "pdf", { skipColumns: [] });
      } else if (format.toLowerCase() === "excel") {
        const { exportTable } = await import("../../helper_functions/ExportData");
        await exportTable(allData, "Requests", "xlsx", { skipColumns: [] });
      } else {
        toast.error(t("Unsupported export format."));
      }
    } catch (err) {
      toast.error(t("Failed to export requests. Please try again."));
    } finally {
      setExporting(false);
    ...
    <Box px={{xs:3,md:0}} pt={1}>
      ...
      <Grid container spacing={2} alignItems="center" mb={1} p = { {xs: 1, md: 2} }>
        <Grid item xs={12} sm={4}>
          <TypographyMD variant="paragraph" label="Requests" color="#424242" fontFamily="Roboto" fontSize={{ xs: "16px", md: "18px" }} fontWeight={600} />
        </Grid>
        <Grid item xs={12} sm={4}>
          <OutlinedInput
            autoComplete="off"
            placeholder={t("Search by type/status...")}
      ...
      {paginatedData.length === 0 ? (
        <Box display={'flex'} flexDirection={'column'} alignItems={'center'} justifyContent="center" py={10}>
                  ...
                  <TypographyMD variant="h2" label={t("No Requests Found!")} color="#A5ADB0" fontFamily="Roboto" fontSize="15px" fontWeight={450} align="center" />
                </Box>
      ) : (
          ...
          <Table sx={{ minWidth: 650, "& .MuiTableCell-root": { padding: { xs: "8px", md: "12px" } },whiteSpace:'nowrap !important' }}>
            <TableHead >
              <TableRow>
                ...
                {["ID", "Type", "Status", "Created Date", "Count"].map((col, idx) => (
                  <TableCell
                    key={idx}
                    align="center"
            ...
            <TableBody>
              {sortedData.map((item) => (
                <TableRow hover key={item.id}>
                  ...
                  <TableCell align="center" sx={{ fontWeight: 400, color: "#172B4D", fontSize: { xs: "12px", md: "14px" } }}>  {formatText(item.type)}
</TableCell>
