...
function Tasks({id}) {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        ...
        const tasksObj = result.data?.data?.tasks?.recent_activity || [];
        const transformed = tasksObj.map((item, idx) => ({
          id: idx + 1,
          status: item.status,
          priority: item.priority,
          created_date: item.created_date,
          count: item.count,
          assigned_workers: item.assigned_workers,
        }));
        setData(transformed);
  ...
  const getSortSymbol = (key) => (sortConfig.key === key ? (sortConfig.direction === "asc" ? <ArrowUpward sx={{ fontSize: "17px" }} /> : <ArrowDownward sx={{ fontSize: "17px" }} />) : <ArrowDownward sx={{ fontSize: "17px" }} />);
  ...
  const fetchAllTasksData = async () => {
    try {
      ...
      const result = await res.json();
      const tasksObj = result.data?.data?.tasks?.recent_activity || [];
      return tasksObj.map((item, idx) => ({
        ID: idx + 1,
        Status: item.status,
        Priority: item.priority,
        "Created Date": item.created_date,
        Count: item.count,
        "Assigned Workers": item.assigned_workers,
      }));
    } catch (err) {
  ...
  const handleExportData = async (format) => {
    ...
    try {
      const allData = await fetchAllTasksData();
      if (!allData.length) {
      ...
      if (format.toLowerCase() === "pdf") {
        ...
        await exportTable(allData, "Tasks", "pdf", { skipColumns: [] });
      } else if (format.toLowerCase() === "excel") {
        const { exportTable } = await import("../../helper_functions/ExportData");
        await exportTable(allData, "Tasks", "xlsx", { skipColumns: [] });
      } else {
        toast.error(t("Unsupported export format."));
      }
    } catch (err) {
      toast.error(t("Failed to export tasks. Please try again."));
    } finally {
      setExporting(false);
    ...
    <Box px={{xs:3,md:0}} pt={1}>
      ...
      <Grid container spacing={2} alignItems="center" mb={1} p = { {xs: 1, md: 2} }>
        <Grid item xs={12} sm={4}>
          <TypographyMD variant="paragraph" label="Tasks" color="#424242" fontFamily="Roboto" fontSize={{ xs: "16px", md: "18px" }} fontWeight={600} />
        </Grid>
        <Grid item xs={12} sm={4}>
          <OutlinedInput
            autoComplete="off"
            placeholder={t("Search by status/priority...")}
      ...
      {paginatedData.length === 0 ? (
       <Box display={'flex'} flexDirection={'column'} alignItems={'center'} justifyContent="center" py={10}>
          ...
          <TypographyMD variant="h2" label={t("No Tasks Found!")} color="#A5ADB0" fontFamily="Roboto" fontSize="15px" fontWeight={450} align="center" />
        </Box>
      ) : (
          ...
          <Table sx={{ minWidth: 650, "& .MuiTableCell-root": { padding: { xs: "8px", md: "12px" } },whiteSpace:'nowrap !important' }}>
            <TableHead >
              <TableRow>
                ...
                {["ID","Status","Priority","Created Date","Count","Assigned Workers"].map((col, idx) => (
                  <TableCell
                    key={idx}
                    align="center"
            ...
            <TableBody>
              {sortedData.map((item) => (
                <TableRow hover key={item.id}>
                  ...
                  <TableCell align="center" sx={{ fontWeight: 400, color: "#172B4D", fontSize: { xs: "12px", md: "14px" } }}>{item.id}</TableCell>
                  ...
                  <TableCell align="center" sx={{ fontWeight: 400, color: "#172B4D", fontSize: { xs: "12px", md: "14px",textTransform:"capitalize" } }}>{item.priority}</TableCell>
                  <TableCell align="center" sx={{ fontWeight: 400, color: "#172B4D", fontSize: { xs: "12px", md: "14px" } }}>
                    <FormatDate inputDate={item.created_date} />
                  </TableCell>
                  <TableCell align="center" sx={{ fontWeight: 400, color: "#172B4D", fontSize: { xs: "12px", md: "14px" } }}>{item.count}</TableCell>
                  <TableCell align="center">
                    <Chip label={item.assigned_workers} size="small" sx={{ backgroundColor: "#F0F9FF", color: "#0369A1", fontWeight: 500 }} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
