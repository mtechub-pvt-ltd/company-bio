...
const UpdateCompanyAdmin = ({
  ...
  console.log("id is ", selectedRowData?.id)

  useEffect(() => {
    if (!open || !selectedRowData) return;
    if (formik.dirty) return; // 🔴 prevents overwrite after update

    console.log("🔴 selectedRowData received in EditCompanyAdmin:", JSON.stringify(selectedRowData, null, 2));
    console.log("🔴 first_name from row:", selectedRowData.first_name);
    console.log("🔴 last_name from row:", selectedRowData.last_name);
    console.log("🔴 email from row:", selectedRowData.email);
    console.log("🔴 phone_number from row:", selectedRowData.phone_number);
    console.log("🔴 business_phone_number from row:", selectedRowData.business_phone_number);
    ...
    formik.setValues({
      ...
      administrator_type: selectedRowData.administrator_type || "",
      email: selectedRowData.email || "",
      phone_number: selectedRowData.phone_number || "",
      legal_name: selectedRowData.legal_name || "",
      trade_name: selectedRowData.trade_name || "",
      business_sector: selectedRowData.business_sector || "",
      company_registration_number: selectedRowData.company_registration_number || "",
      business_email: selectedRowData.business_email || "",
      business_phone_number: selectedRowData.business_phone_number || "",
      // business_type: selectedRowData.business_type || "",
      business_activity: selectedRowData.business_activity || "",
      primary_color: selectedRowData.primary_color || "#006EC2",
      secondary_color: selectedRowData.secondary_color || "#2C384C",
      ...
      region_code: selectedRowData.region_code || selectedRowData.region || "",
  ...
  // Add this new effect hook right after the existing useEffect that checks selectedRowData
  // Place it BEFORE the formik definition

  // Helper function to extract correct image URL from doubled URL
  const extractCorrectImageUrl = (urlString) => {
    if (!urlString) return null;

    // If the URL contains the API URL twice (doubled URL), extract the inner one
    const apiPattern = /https?:\/\/[^/]+\/api\/uploads\//;
    const match = urlString.match(apiPattern);

    if (match) {
      // Find the last occurrence of the api pattern and extract from there
      const lastIndex = urlString.lastIndexOf(match[0]);
      if (lastIndex !== -1) {
        return urlString.substring(lastIndex);
      }
    }
  ...
  const fetchCompanyAdminDetails = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${url}company-admins/${selectedRowData?.id}`, {
        method: "GET",
        headers: {
      ...
      if (data.error === false && data.data?.company_admin) {
        const admin = data.data.company_admin;
        const rawData = admin.raw_data;

        // Extract correct image URLs from doubled URLs
        const profilePictureUrl = extractCorrectImageUrl(
          admin.account_details?.profile_picture_url ||
          admin.account_details?.profile_picture
        );
        const companyLogoUrl = extractCorrectImageUrl(
          admin.company_details?.company_logo_url ||
          admin.company_details?.company_logo
        );
        const adminDocUrl = extractCorrectImageUrl(
          admin.company_details?.admin_document_url
        );
        const companyDocUrl = extractCorrectImageUrl(
          admin.company_details?.company_document_url
        );

        console.log("Extracted Profile URL:", profilePictureUrl);
        console.log("Extracted Logo URL:", companyLogoUrl);

        // Set preview images with corrected URLs
        if (profilePictureUrl) {
          setPreviewUrl(profilePictureUrl);
        }
        if (companyLogoUrl) {
          setPreviewLogo(companyLogoUrl);
        }

        // Set admin document preview
        if (adminDocUrl) {
          setPreviewAdminDoc(adminDocUrl);
          setAdminDocFileName("admin-document.pdf");
        }

        // Set company document preview if exists
        if (companyDocUrl) {
          setPreviewCompanyDoc(companyDocUrl);
          setCompanyDocFileName("company-document.pdf");
        }

        // Set map position
        if (admin.admin_address_details?.latitude && admin.admin_address_details?.longitude) {
          setMapPosition({
            lat: parseFloat(admin.admin_address_details.latitude),
            lng: parseFloat(admin.admin_address_details.longitude),
          });
        }

        // Fill all form fields with API response data
        formik.setValues({
          first_name: rawData.first_name || "",
          middle_name: rawData.middle_name || "",
          last_name: rawData.last_name || "",
          dob: rawData.dob ? rawData.dob.split("T")[0] : "",
          administrator_type: rawData.administrator_type || "",
          admin_document_url: adminDocUrl || "",
          email: rawData.registered_email || "",
          phone_number: rawData.phone || "",
          legal_name: rawData.legal_name || "",
          trade_name: rawData.trade_name || "",
          business_sector: rawData.business_sector || "",
          company_registration_number: rawData.business_registration_no || "",
          business_email: rawData.business_email || "",
          business_phone_number: rawData.business_phone_no || "",
          // business_type: rawData.business_type || "",
          business_activity: rawData.business_activity || "",
          business_color: rawData.business_color || "#1976d2",
          primary_color: rawData.primary_color || "#006EC2",
          secondary_color: rawData.secondary_color || "#2C384C",
          business_address: rawData.business_address || "",
          company_document_url: companyDocUrl || "",
          country: rawData.country || "",
          province: rawData.community_province || "",
          postal_code: rawData.postal_code || "",
          community: rawData.community || "",
          city: rawData.city || "",
          street_address: rawData.street_address || "",
          region_code: rawData.region_code || "",
          account_executives: rawData.account_executive_id?.toString() || "",
          subscription_type: rawData.subscription || "",
          // territory_zone: rawData.ae_territory_zone || [],
        ...
        setLoading(false);
        showToast(toast.error, data, t("Failed to fetch company admin details"));
      }
    } catch (error) {
      setLoading(false);
      console.error("Error fetching company admin details:", error);
      toast.error(t("Failed to load company admin details"));
    }
  };

  // Add this useEffect to fetch data when modal opens with a selected row
  useEffect(() => {
    if (open && selectedRowData?.id) {
      fetchCompanyAdminDetails(selectedRowData.id);
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const InsertAPIURL = `${url}super-admin/company-admins/${selectedRowData?.id}`;

        console.log("📋 ===== BEFORE FormData Preparation =====");
        ...
        console.log("🔵 Business Phone Number:", values.business_phone_number);
        ...
        console.log("🔵 Territory Zone:", values.territory_zone);
        console.log("🔵 Country:", values.country);
        console.log("🔵 City:", values.city);
        console.log("🔵 Province:", values.province);
        console.log("🔵 Street Address:", values.street_address);
        console.log("🔵 Region Code:", values.region_code);
        console.log("🔵 Account Executives:", values.account_executives);
        console.log("🔵 Latitude:", values.latitude);
        console.log("🔵 Longitude:", values.longitude);
        console.log("🔵 API URL:", InsertAPIURL);
        ...
        const fieldNameMapping = {
          ...
          admin_document_url: "admin_document_url",
          company_document_url: "company_document_url",
        };

        Object.entries(values).forEach(([key, val]) => {
          // Get the backend field name (or use the original key if not in mapping)
          const backendKey = fieldNameMapping[key] || key;

          if (key === "territory_zone" && Array.isArray(val)) {
            formData.append(backendKey, JSON.stringify(val));
            console.log(`✅ Appended ${backendKey} (from ${key}):`, JSON.stringify(val));
          } else if (key === "latitude" || key === "longitude") {
            const numericValue = val !== null && val !== undefined && !isNaN(val) ? Number(val) : null;
            if (numericValue !== null) {
              formData.append(backendKey, numericValue);
              console.log(`✅ Appended ${backendKey} (from ${key}):`, numericValue);
            }
          } else {
            const valueToAppend = safeValue(val);
            formData.append(backendKey, valueToAppend);
            console.log(`✅ Appended ${backendKey} (from ${key}):`, valueToAppend);
          }
        });
        ...
        console.log("✅ Appended account_executive_association:", isNaN(execValue) ? "-" : execValue);

        if (profileImageUrl) {
          formData.append("profile_picture_url", profileImageUrl);
          console.log("✅ Appended profile_picture_url:", profileImageUrl);
        }
        if (finalLogoUrl) {
          formData.append("company_logo_url", finalLogoUrl);
          console.log("✅ Appended company_logo_url:", finalLogoUrl);
        }
        ...
        console.log("=========================================");
        console.log("📋 Keys being sent in FormData:");
        console.log("   - first_name:", formData.has('first_name') ? "✅ YES" : "❌ NO");
        console.log("   - last_name:", formData.has('last_name') ? "✅ YES" : "❌ NO");
        console.log("   - email:", formData.has('email') ? "✅ YES" : "❌ NO");
        console.log("   - phone_number:", formData.has('phone_number') ? "✅ YES" : "❌ NO");
        console.log("   - business_phone_number:", formData.has('business_phone_number') ? "✅ YES" : "❌ NO");
        console.log("   - street_address:", formData.has('street_address') ? "✅ YES" : "❌ NO");
        console.log("   - region_code:", formData.has('region_code') ? "✅ YES" : "❌ NO");
        console.log("=========================================");
        console.log("🚀 Making PUT request to:", InsertAPIURL);
        ...
        const payloadToSend = {
          ...values,
          phone_number: values.phone_number, // Keep original field name
          business_phone_number: values.business_phone_number, // Keep original field name
          profile_picture_url: profileImageUrl || values.profile_picture_url || selectedRowData?.profile_picture,
          company_logo_url: finalLogoUrl || values.company_logo_url || selectedRowData?.logo,
        };

        console.log("📦 ===== JSON Payload to Send =====");
        console.log("  Profile Picture URL:", payloadToSend.profile_picture_url);
        console.log("  Company Logo URL:", payloadToSend.company_logo_url);
        console.log("  All other fields prepared");
        console.log("=========================================");
        console.log("🚀 Making PUT request to:", InsertAPIURL);
        ...
        console.log("📨 API RESPONSE DATA FIELDS:");
        if (data.data) {
          console.log("   - first_name:", data.data.first_name);
          console.log("   - last_name:", data.data.last_name);
          console.log("   - email:", data.data.email);
          console.log("   - phone:", data.data.phone);
          console.log("   - business_phone:", data.data.business_phone);
          console.log("   - street_address:", data.data.street_address);
          console.log("   - region_code:", data.data.region_code);
        }
        ...
        if (data.error === true || data.error === "true") {
          ...
          // Update formik with the response data to reflect changes
          if (data.data) {
            formik.setValues({
              ...
              administrator_type: data.data.administrator_type || "",
              email: data.data.email || "",
              phone_number: data.data.phone || "",
              legal_name: data.data.legal_name || "",
              trade_name: data.data.trade_name || "",
              business_sector: data.data.business_sector || "",
              company_registration_number: data.data.company_registration_number || "",
              ...
              business_activity: data.data.business_activity || "",
              business_color: data.data.business_color || "#1976d2",
              primary_color: data.data.primary_color || "#006EC2",
              secondary_color: data.data.secondary_color || "#2C384C",
              business_address: data.data.business_address || "",
              company_document_url: data.data.company_document_url || "",
              ...
              account_executives: data.data.account_executive_id?.toString() || "",
            ...
            // Update preview images with response data
          ...
          showToast(toast.success, data, t("Company Admin updated successfully"));
          if (onSuccess) onSuccess();
          onClose(); // ✅ Close modal via parent, don't call handleModalClose
        }
      } catch (error) {
        setLoading(false);
  ...
  useEffect(() => {
    if (!open || !selectedRowData) return;
    if (formik.dirty) return; // 🔴 prevents overwrite after user makes changes
    ...
    formik.setValues({
      ...
      // business_type: selectedRowData.business_type || "",
      business_activity: selectedRowData.business_activity || "",
      business_color: selectedRowData.business_color || "#1976d2",
      primary_color: selectedRowData.primary_color || "#006EC2",
      secondary_color: selectedRowData.secondary_color || "#2C384C",
      business_address: selectedRowData.business_address || "",
      company_document_url: selectedRowData.company_document_url || "",
      country: selectedRowData.country || "",
      province: selectedRowData.province || "",
      city: selectedRowData.city || "",
      street_address: selectedRowData.street_address || selectedRowData.street || "",
      ...
      // territory_zone: Array.isArray(selectedRowData.territory_zone) ? selectedRowData.territory_zone : [],
      latitude: selectedRowData.latitude ? parseFloat(selectedRowData.latitude) : null,
      longitude: selectedRowData.longitude ? parseFloat(selectedRowData.longitude) : null,
      account_executives: selectedRowData.account_executive_association?.toString() || "",
      subscription_type: selectedRowData.subscription_type || "",
    });

    // Pre-fill images
    if (selectedRowData.profile_picture) {
      setPreviewUrl(selectedRowData.profile_picture);
    }
    if (selectedRowData.logo || selectedRowData.company_logo_url) {
      setPreviewLogo(selectedRowData.logo || selectedRowData.company_logo_url);
    }

    // Set map position
    if (selectedRowData.latitude && selectedRowData.longitude) {
      setMapPosition({
        lat: parseFloat(selectedRowData.latitude),
        lng: parseFloat(selectedRowData.longitude),
      });
    }
  }, [selectedRowData, open]);
                      ...
                      <div style={{ marginBottom: { xs: "10px", sm: "15px" }, marginTop: { xs: "5px", sm: "10px" } }}>
                        ...
                        /> */}
                            <Inputfield
                              ...
                              type="date"
                              onClick={() => {
                                dobRef.current?.showPicker?.();   // open calendar on full box click
                              }}
                              onChngeterm={(e) => {
                          ...
                          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mb: 2 }}>
                            ...
                            >
                              <MenuItem value="" disabled>
                                {loadingManualCountries ? t("placeholders.loadingCountries") : t("placeholders.selectCountry")}
                              </MenuItem>
                            ...
                            >
                              <MenuItem value="" disabled>
                                {!selectedCountry ? t("placeholders.selectCountryFirst") :
                                  loadingStates ? t("placeholders.loadingStates") : t("placeholders.selectState")}
                              </MenuItem>
                              {states.map((state) => (
                            ...
                            >
                              <MenuItem value="" disabled>
                                {!selectedState ? t("placeholders.selectStateFirst") :
                                  loadingCities ? t("placeholders.loadingCities") : t("placeholders.selectCity")}
                              </MenuItem>
                              {cities.map((city) => (
