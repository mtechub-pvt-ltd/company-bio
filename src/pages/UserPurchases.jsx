...
function UserPurchases({ user_id }) {
const {t}=useTranslation()
    ...
    const handleFilterSelect = (filter) => {
        ...
        if (filter === 'A-Z') {
            sortedRows.sort((a, b) =>
                (a.order_details?.order_id?.toString() || '').localeCompare(b.order_details?.order_id?.toString() || '')
            );
        } else if (filter === 'Newest') {
            sortedRows.sort((a, b) => {
                const dateB = new Date(b.order_details?.order_date?.slice(0, 10));
                const dateA = new Date(a.order_details?.order_date?.slice(0, 10));
                return dateB - dateA;
            });
            ...
            sortedRows.sort((a, b) =>
                (b.order_details?.order_id?.toString() || '').localeCompare(a.order_details?.order_id?.toString() || '')
            );
        }

        const filteredData = sortedRows.filter(item =>
            (item?.order_details?.order_id || '').toString().toLowerCase().includes(searchTerm.toLowerCase())
        );
    ...
    const filteredData = allPurchases?.filter(item =>
        (item?.order_details?.order_id || '').toString().toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            <Box sx={{ backgroundColor: "white", borderRadius: "14px", mt: 2, ml: 2, mr: 2 }}>
                <Grid container spacing={0}  >
                    <Grid xs={6} md={6} align="center" p={1}>
                        <TypographyMD variant='h2' label="Purchases" color="#424242" fontFamily="Nunito Sans" marginLeft={0} fontSize="25px" fontWeight={550} align="left" />
                    </Grid>
                                    ...
                                    <Table sx={{ minWidth: { xs: "100px", md: '250px' } }} aria-label="simple table">
                                        <TableHead style={{ backgroundColor: '#F4F6FA' }}>
                                            <TableRow  >
                                                ...
                                                <TableCell align="center" sx={{ fontWeight: "bold", color: "#424242", fontFamily: "Nunito Sans", fontSize: "15px" }}> TOTAL AMOUNT </TableCell>
                                                <TableCell align="center" sx={{ fontWeight: "bold", color: "#424242", fontFamily: "Nunito Sans", fontSize: "15px" }}> ORDER STATUS </TableCell>
                                                <TableCell align="center" sx={{ fontWeight: "bold", color: "#424242", fontFamily: "Nunito Sans", fontSize: "15px" }}> PAYMENT TYPE </TableCell>
                                            ...
                                            {displayedRows?.map((item) => (
                                                <TableRow>
                                                    ...
                                                    <TableCell align="center" sx={{ fontWeight: "normal", color: "#545454", fontFamily: "Nunito Sans", fontSize: "13px" }} >
                                                        # {highlightMatch((item?.order_details?.order_id || '').toString(), searchTerm)}
                                                    </TableCell>

                                                    <TableCell align="center" sx={{ fontWeight: "normal", color: "#545454", fontFamily: "Nunito Sans", fontSize: "13px" }} >$ {item?.order_details?.total_amount}</TableCell>
                                                    <TableCell align="center" sx={{ fontWeight: "normal", color: "#545454", fontFamily: "Nunito Sans", fontSize: "13px" }} >
                                                        <Box
                                                            sx={{
                                                                ...
                                                                textTransform: "capitalize",
                                                                bgcolor:
                                                                    item?.order_details?.order_status === "completed"
                                                                        ? "#00b894"
                                                                        : item?.order_details?.order_status === "order_placed"
                                                                            ? "#e67e22"
                                                                            : item?.order_details?.order_status === "processing"
                                                                                ? "#6c5ce7"
                                                                                : item?.order_details?.order_status === "cancelled"
                                                                                    ? "#ff6b6b"
                                                                                    : "#b2bec3"
                                                            }}
                                                        >
                                                            {item?.order_details?.order_status?.replace("_", " ")}
                                                        </Box></TableCell>
                                                    <TableCell align="center" sx={{ fontWeight: "normal", color: "#545454", fontFamily: "Nunito Sans", fontSize: "13px" }} >{item?.order_details?.payment_type}</TableCell>
                                                    <TableCell align="center" sx={{ fontWeight: "normal", color: "#545454", fontFamily: "Nunito Sans", fontSize: "13px" }} >
                                                        <>
                                                            {new Date(item?.order_details?.order_date
