...
const handleFilterSubmit = (e) => {
  ...
  const getAllExecutives = async () => {
    ...
    try {
      ...
      const execs =
        data?.data?.account_executives?.map((exec) => ({
          value: exec.id,
          label: exec.full_name,
        })) || [];

      setAccountExecutives([{ value: "all", label: "All" }, ...execs]);
    } catch (error) {
      toast.error("Something went wrong! Please try again.");
    } finally {
...
const handleExportData = async (format) => {
  ...
  try {
    ...
    if (!allData.length) {
      toast.error("No commissions available for export.");
      return;
    }

    const skipColumns = ["id"]; // skip internal IDs if needed

    // map missing values to localized N/A
    const mapped = allData.map((row) => {
      const out = {};
      Object.keys(row).forEach((k) => {
        const v = row[k];
        out[k] = (v === null || v === undefined || (typeof v === "string" && v.trim() === "") || v === "null" || v === "undefined") ? t("N/A") : v;
      });
      return out;
    });

    if (format.toLowerCase() === "pdf") {
      await exportTable(mapped, t("Commissions Report"), "pdf", { skipColumns });
    } else if (format.toLowerCase() === "excel") {
      await exportTable(mapped, t("Commissions Report"), "xlsx", { skipColumns });
    } else {
...
const fetchAllCommissionsForExport = async () => {
  ...
  icon={
    exporting ? <CircularProgress size={15} /> : <img src={exportIcon} alt="Export" style={{ width: 30 }} />
  }
  options={[
    { label: "PDF", icon: pdfIcon },
    { label: "Excel", icon: csvIcon },
  ]}
  disabled={exporting} // prevent multiple clicks
  sx={{
    ...fieldCommonSx,
    px: 2,
...
/>
                    ...
                    >
                      ...
                      <div style={{ marginBottom: "5px", marginTop: "10px" }}>
                        ...
                        >
                          <FormControl
                            size="small"
                            sx={{ minWidth: "100%" }} // same as executive=true behavior
                          >
                            <Select
                              value={draftExecId}
                              onChange={(e) => setDraftExecId(e.target.value)}
                              variant="outlined"
                              sx={{
                                height: 35,
                                border: "none", // remove border
                                "& .MuiOutlinedInput-notchedOutline": {
                                  border: "none", // remove outlined border
                                },
                                ...fieldCommonSx,
                              }}
                            >
    ...
    // Ensure the year part is max 4 digits (format: YYYY-MM-DD)
    ...
    setDraftStartDate(value);
                              ...
                              style={{
                                ...
                                border: "2px solid rgba(9, 30, 66, 0.14)",
                                backgroundColor: "#fff",
                                fontSize: "14px", // For responsive font sizes, you can use media queries in CSS
                                color: "rgba(27, 27, 27, 0.67)",
                                padding: "0 8px",
                                boxSizing: "border-box",
                              }}
                              onFocus={(e) =>
                                (e.target.style.borderColor = "#006EC2")
                              }
                              onBlur={(e) =>
...
<input
  ...
  onChange={(e) => {
    let value = e.target.value;
    // The input type="date" format is YYYY-MM-DD
    const [year, month, day] = value.split("-");
    if (year && year.length > 4) {
      const trimmedYear = year.slice(0, 4);
      value = [trimmedYear, month, day].filter(Boolean).join("-");
